...
from utils.plot_utils import plot_pose_gains, plot_value_psnr, plot_combined_psnr, dump_realtime_dataset
...
def rgbd_slam(config: dict, ros_handler_config: dict):
    ...
    # Dump the dataset if required
    if ros_handler._dump_data:
    ...
    gain_list = np.array([g["gain"] for g in ros_handler.gain_psnr_arr])
    
    avg_psnr = psnr_list.mean()
    avg_eig  = eig_list.mean()
    avg_sil  = sil_list.mean()
    avg_gain = gain_list.mean()
    ...
    print("Average gain (before mapping): {:.2f}".format(avg_gain))
    ...
    # Plot <value> vs PSNR
    plot_value_psnr(psnr_list, sil_list, axis_name="SIL", prefix="psnr_sil", save_dir=output_dir + '/psnr_plots')
    plot_value_psnr(psnr_list, eig_list, axis_name="EIG", prefix="psnr_eig", save_dir=output_dir + '/psnr_plots')
    plot_value_psnr(psnr_list, gain_list, axis_name="GAIN", prefix="psnr_gain", save_dir=output_dir + '/psnr_plots')
    plot_combined_psnr(psnr_list, sil_list, eig_list, prefix="psnr_combined", save_dir=output_dir + '/psnr_plots')

    # Store averages before mapping
    np.savetxt(f"{output_dir}/running_metrics_{time.time_ns()}.csv",
               np.column_stack((sil_list, eig_list, psnr_list, gain_list)),
               delimiter=",", header="sil,eig,psnr,gain", comments="")
